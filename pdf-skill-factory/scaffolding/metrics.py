...
# ---------------------------------------------------------------------------
# v0.3.0 metrics (Train-on-Use: learning_candidate + mode separation)
# ---------------------------------------------------------------------------

def learning_candidate_schema_valid(candidate: dict) -> float:
    """Validate one learning_candidate against the required fields + enums.
    Returns 1.0 if valid, 0.0 otherwise. Does NOT require jsonschema lib —
    checks required fields + enum values inline."""
    required = [
        "candidate_id", "created_at", "skill_version", "source_session",
        "input_file_type", "document_type_detected", "task_type", "trigger",
        "observed_issue", "risk_level", "suggested_action",
        "requires_human_review", "status",
    ]
    for f in required:
        if f not in candidate or candidate[f] in (None, ""):
            # status may be set; empty string is invalid
            if candidate.get(f) is False or candidate.get(f) == 0:
                continue  # 0/False are valid for some fields
            return 0.0
    import re as _re
    if not _re.match(r"^LC-\d{4}-\d{3}$", str(candidate["candidate_id"])):
        return 0.0
    if candidate["risk_level"] not in ("low", "medium", "high"):
        return 0.0
    if candidate["suggested_action"] not in ("ignore", "add_memory", "create_regression", "train_session", "proposal_required"):
        return 0.0
    if candidate["status"] not in ("open", "accepted", "rejected", "converted_to_regression", "converted_to_training_session"):
        return 0.0
    if candidate["trigger"] not in (
        "new_document_type", "citation_defect", "table_defect", "abstention_defect",
        "numeric_defect", "legal_structure_defect", "forecast_horizon_defect",
        "low_parse_confidence", "ocr_required_unavailable", "novel_pattern",
        "user_reported_error", "self_check_warning",
    ):
        return 0.0
    # Privacy: evidence must be ≤300 chars
...
def learning_candidates_dir_valid(open_dir) -> float:
    """Validate every *.json in learning_candidates/open/. Returns fraction valid.
    `open_dir` is a pathlib.Path."""
    if not open_dir.exists():
        return 1.0  # vacuous — no candidates yet
    files = sorted(open_dir.glob("LC-*.json"))
    if not files:
        return 1.0
    ...
    for f in files:
        try:
            ...
            if learning_candidate_schema_valid(cand) == 1.0:
                valid += 1
        except Exception:
            pass
...
def run_mode_no_self_patch(runtime_state: dict) -> float:
    """Check that a RUN-mode session did not modify runtime files.
    `runtime_state` = {'skill_md_modified': bool, 'policies_modified': bool,
                       'scripts_modified': bool, 'memory_modified': bool,
                       'version_bumped': bool, 'mode': 'RUN'|'TRAIN'|'RELEASE'}.
    Returns 1.0 if RUN mode left runtime untouched, else 0.0."""
    if runtime_state.get("mode") != "RUN":
        return 1.0  # only enforced in RUN
    forbidden = (
        runtime_state.get("skill_md_modified")
        or runtime_state.get("policies_modified")
        or runtime_state.get("scripts_modified")
        or runtime_state.get("memory_modified")
        or runtime_state.get("version_bumped")
    )
    return 0.0 if forbidden else 1.0


def memory_not_updated_without_train(memory_state: dict) -> float:
    """Check that skill_memory was not written by RUN mode.
    `memory_state` = {'last_writer_mode': 'RUN'|'TRAIN'|'RELEASE'|None}.
    Returns 1.0 if last writer was NOT RUN (or no write yet), else 0.0."""
    last = memory_state.get("last_writer_mode")
    if last is None:
        return 1.0
    return 0.0 if last == "RUN" else 1.0


def candidate_created_for_trigger(candidate_created: bool, trigger_expected: bool) -> float:
    """Returns 1.0 if candidate-creation behavior matches expectation."""
    return 1.0 if bool(candidate_created) == bool(trigger_expected) else 0.0
