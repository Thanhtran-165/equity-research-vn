...
import metrics  # noqa: E402  (for v0.3.0 helper functions: learning_candidates_dir_valid, run_mode_no_self_patch, candidate_created_for_trigger, memory_not_updated_without_train)
...
# ---------------------------------------------------------------------------
# v0.3.0 — Train-on-Use tests
# ---------------------------------------------------------------------------

SKILL_ROOT = SCAFFOLDING_DIR.parent / "pdf-evidence"
LC_OPEN_DIR = SKILL_ROOT / "learning_candidates" / "open"
LC_SCHEMA = SKILL_ROOT / "learning_candidates" / "schema.json"


def test_v030_learning_candidate_schema_valid():
    """v0.3.0 #1 — every existing learning candidate validates against schema rules."""
    assert LC_SCHEMA.exists(), "learning_candidates/schema.json missing"
    val = metrics.learning_candidates_dir_valid(LC_OPEN_DIR)
    assert val == 1.0, (
        f"learning_candidate schema validation failed: {val:.2f} < 1.0 "
        f"(one or more candidates in {LC_OPEN_DIR} are invalid)"
    )


def test_v030_run_mode_does_not_self_patch():
    """v0.3.0 #2 — RUN mode must not modify runtime files."""
    # Simulate a RUN-mode session state
    clean_run = {
        "mode": "RUN",
    ...
    assert metrics.run_mode_no_self_patch(clean_run) == 1.0, (
        "RUN mode with no modifications should pass"
    )
    # A RUN session that patched SKILL.md must fail this gate
    bad_run = dict(clean_run, skill_md_modified=True)
    assert metrics.run_mode_no_self_patch(bad_run) == 0.0, (
        "RUN mode that modified SKILL.md must fail the no-self-patch gate"
    )


def test_v030_candidate_created_for_new_doc_type():
    """v0.3.0 #3 — RUN mode must create a candidate when document_type is unknown."""
    # Simulate: classify returned a doc_type not in the policy enum
    known_types = {
        "digital_text", "scanned", "mixed", "table_heavy", "legal",
        "financial", "academic", "policy", "slide_export", "low_quality_ocr",
    }
    unknown_type = "central_bank_circular"
    assert unknown_type not in known_types, "fixture: unknown_type must be outside enum"
    # The skill SHOULD create a candidate when unknown_type detected → expectation True
    assert metrics.candidate_created_for_trigger(True, True) == 1.0
    # If skill failed to create one → expectation mismatch → fail
    assert metrics.candidate_created_for_trigger(False, True) == 0.0


def test_v030_candidate_created_for_citation_defect():
    """v0.3.0 #4 — RUN mode must create a candidate when a citation lacks table_id."""
    # Simulate: a citation on a table page missing table_id
    citation = {"file": "x", "page": 1, "section": "tbl", "quote": "...", "table_id": None}
    page_has_table = True
    defect = page_has_table and not citation.get("table_id")
    assert metrics.candidate_created_for_trigger(True, defect) == 1.0
    assert metrics.candidate_created_for_trigger(False, defect) == 0.0


def test_v030_memory_not_written_directly_from_run():
    """v0.3.0 #5 — skill_memory must not be written by RUN mode (only TRAIN)."""
    # RUN mode never writes memory
    assert metrics.memory_not_updated_without_train({"last_writer_mode": "RUN"}) == 0.0, (
        "RUN mode writing skill_memory violates mode separation"
    )
    # TRAIN mode may write memory
    assert metrics.memory_not_updated_without_train({"last_writer_mode": "TRAIN"}) == 1.0
    # No write yet → ok
    assert metrics.memory_not_updated_without_train({"last_writer_mode": None}) == 1.0


def test_v030_release_mode_only_mode_that_bumps_version():
    """v0.3.0 #6 — only RELEASE mode may bump metadata.version."""
    # Simulated mode→bump authorization
    def can_bump(mode: str) -> bool:
        return mode == "RELEASE"
    assert can_bump("RUN") is False, "RUN must not bump version"
    assert can_bump("TRAIN") is False, "TRAIN must not bump version"
    assert can_bump("RELEASE") is True, "RELEASE is the only mode that may bump"
