...
def run_fundamental(request: FundamentalRequest) -> Any:
    ...
    # Near-zero equity relative to assets (equity/assets < 5%)
    if equity is not None and assets is not None and assets != 0:
        eq_ratio = abs(equity) / abs(assets)
        if eq_ratio < 0.05 and equity > 0:
            for m in metric_results:
                if m.metric_id in ("ROE", "BVPS"):
                    warning = f"EXTREME_EQUITY_RATIO_WARNING: equity/assets={eq_ratio:.4f} < 5%"
                    if warning not in m.warnings:
                        m.warnings.append(warning)
    ...
    # PE applicability: NOT_APPLICABLE when EPS <= 0 (negative or zero)
    eps_val_for_pe = eps_result.value if eps_result else None
    pe_applicability = "NOT_APPLICABLE" if (eps_val_for_pe is not None and eps_val_for_pe <= 0) else "APPLICABLE"
    ...
    downstream_export = {
        "EPS_basic": {
            ...
            "quality_status": shares_metric.quality_status if shares_metric else "VALID",
