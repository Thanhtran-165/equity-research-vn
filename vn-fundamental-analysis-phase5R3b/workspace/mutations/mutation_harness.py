...
def _post_flip_eps_sign(res):
    ...
    for m in res.output.metric_results:
        if m.metric_id == "EPS_BASIC" and m.value is not None and m.value < 0:
            m.value = abs(m.value); m.status = MetricStatus.VALID.value
    return res
...
def _set_derived_shares(req):
    req = copy.deepcopy(req)
    req.metrics["shares_outstanding"].quality_status = "DERIVED_INPUT"
    return req

def _set_negative_npat(req):
    req = copy.deepcopy(req)
    req.metrics["net_income"].values = [5000, 5500, -3000]  # latest year negative
    return req
...
def _set_near_zero_equity(req):
    req = copy.deepcopy(req)
    req.metrics["equity"].values = [500, 400, 0.5]
...
def _post_strip_bvps_warning(res):
    ...
    Pre-condition: BVPS value is above threshold (clean fixture with high BVPS).
    """
    res = copy.deepcopy(res)
    for m in res.output.metric_results:
        if m.metric_id == "BVPS":
...
MUTATIONS: List[Mutation] = [
    ...
    Mutation("MUT-FUND-033", "CRITICAL", "negative EPS suppressed to INPUT_INCOMPLETE (post-run)", "DOWNSTREAM_EXPORT_BLOCKED", "APPLICABILITY_ENGINE", "negative EPS suppression", _set_negative_npat, post_run=_post_suppress_negative_eps),
    Mutation("MUT-FUND-034", "HIGH", "negative EPS sign flipped (post-run)", "EPS_SIGN_INCONSISTENT", "FORMULA_ENGINE", "negative EPS sign", _set_negative_npat, post_run=_post_flip_eps_sign),
    Mutation("MUT-FUND-035", "HIGH", "negative equity ROE marked VALID (post-run)", "NEGATIVE_EQUITY_NOT_MEANINGFUL", "NEGATIVE_VALUE_ENGINE", "negative equity ROE", _set_negative_equity, post_run=_post_force_roe_valid),
    Mutation("MUT-FUND-036", "MEDIUM", "near-zero equity warning removed (post-run)", "REQUIRED_WARNING_REMOVED", "QUALITY_ENGINE", "extreme equity ratio warning", _set_near_zero_equity, post_run=_post_strip_equity_warning),
    Mutation("MUT-FUND-037", "MAJOR", "DERIVED_INPUT flag removed (post-run)", "QUALITY_STATUS_MISMATCH", "PROVENANCE_ENGINE", "derived input quality", lambda r: _set_derived_shares(r), post_run=_post_strip_derived_flag),
    Mutation("MUT-FUND-038", "CRITICAL", "circular EPS oracle accepted (post-run)", "CIRCULAR_ORACLE_DETECTED", "VERIFIER", "circular oracle prohibition", lambda r: r, post_run=_post_inject_circular_oracle),
]
