#!/usr/bin/env python3
"""VN100 rebuild runner — chạy vn100_v2.py cho list ticker + record tracker."""
import json, shutil, os, subprocess, sys, time

BATCH = sys.argv[1] if len(sys.argv) > 1 else 'all'  'all' or '1'/'2'/'3'
...
for T, SEC in rows:
    ...
    r = subprocess.run(['python3','/tmp/vn100_v2.py',T,SEC], capture_output=True, text=True)
    ...
    for row in tr['tickers']:
        if row['ticker']==T:
            ...
            row['fail_reqs'] = fails
            row['notes'] = f"v2 rebuild: price {res.get('price',0)}, mcap {res.get('mcap',0)} tỷ, tech {res.get('tech_score',0)} {res.get('verdict','')}"
            break
...
if recalls:
    ...
    print(f'≥60/74: {sum(1 for r in recalls if r>=60)} mã')
