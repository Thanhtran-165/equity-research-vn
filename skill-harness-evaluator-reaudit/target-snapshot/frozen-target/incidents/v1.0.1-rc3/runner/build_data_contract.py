...
def main():
    ...
    # === RC2.1 (ERVN-SECTOR-APPLICABILITY-001): NOT_APPLICABLE semantics ===
    # Insurance/banking sectors have no generic "Sales" column. Previously the
    # builder emitted revenue=[0,0,0,0,0] which conflated "field-not-applicable"
    # with "value is zero". Per owner directive 2026-07-19 §2, the correct
    # encoding is null+NOT_APPLICABLE+applicability_rule, so downstream consumers
    # (valuation, growth, charts, narrative) can distinguish and skip gracefully.
    REV_APPLICABILITY_RULE = "INSURANCE_REVENUE_NOT_GENERIC_SALES"
    revenue_is_na = (len(revenue) == 0)  # no year had a resolved revenue value
    if revenue_is_na:
        rev_arr = None  # whole array is N/A
        revenue_status = "NOT_APPLICABLE"
        revenue_applicability_rule = REV_APPLICABILITY_RULE
    else:
        rev_arr = [round(revenue.get(str(y), 0) / 1e9, 2) for y in years_asc]
        revenue_status = "VALID"
        revenue_applicability_rule = None
    ...
    contract = {
      # RC2.1: bumped contract schema version (was implicit "0.1" before)
      "contract_schema_version": "0.2.0-rc2",
      ...
      # RC2.1: explicit field applicability block — null+NOT_APPLICABLE encoding
      "field_applicability": {
        "revenue": {
          "status": revenue_status,
          "applicability_rule": revenue_applicability_rule,
          # When status=NOT_APPLICABLE:
          #   - financials.revenue array is null (NOT [0,0,0,0,0])
          #   - downstream valuation/growth/charts MUST skip revenue-dependent metrics
          #   - narrative MUST NOT claim "doanh thu = 0"
          # When status=VALID: financials.revenue is the normal tỷ-VND array
        },
      },
      ...
      "_provenance": {
        ...
        "unit": "tỷ VND (financials arrays); VND (price, eps); null=NOT_APPLICABLE",
        ...
        "rc2_changes": [
          "ERVN-SECTOR-APPLICABILITY-001: revenue NOT_APPLICABLE encoding for insurance/banking",
          "contract_schema_version bumped 0.1→0.2.0-rc2",
        ],
      }
    }
