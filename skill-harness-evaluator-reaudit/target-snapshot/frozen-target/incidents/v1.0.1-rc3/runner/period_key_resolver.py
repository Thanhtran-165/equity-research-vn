...
def _try_strategy_1_json(source_pack: str, expected_year_count: int,
    ...
    for y in parsed_years:
        y_data = data[y]
        # Find income row matching revenue. If revenue not in fs data (insurance/banking),
        # fall back to net_profit or eps for row matching.
        inc_idx = _find_row_matching_value(income_rows, y_data.get("revenue"),
                                            aliases=["sales", "net sales", "revenue"])
        if inc_idx is None:
            # RC3: fallback to net_profit for sectors without generic revenue
            for fallback_key, fallback_aliases in [
                ...
                ("Net profit attributable to shareholders of the group",
                 ["net profit attributable", "attributable to parent"]),
                ("EPS basic (VND)", ["eps basic"]),
            ]:
                fb_val = y_data.get(fallback_key)
                if fb_val is not None:
                    inc_idx = _find_row_matching_value(income_rows, fb_val, aliases=fallback_aliases)
        ...
        if bal_idx is None:
            # Fallback: use equity
            for fb_key, fb_aliases in [("equity", ["owner's equity", "equity"]),
                                        ("total_equity", ["total equity"])]:
                fb_val = y_data.get(fb_key)
                if fb_val is not None:
                    bal_idx = _find_row_matching_value(balance_rows, fb_val, aliases=fb_aliases)
        ...
        if inc_idx is None:
            raise ResolverError("INCONSISTENT_ACROSS_STATEMENTS",
                                f"Strategy 1: year {y} no matching income row found "
                                f"(tried revenue + net_profit + eps fallbacks)")
