...
def run_fundamental(request: FundamentalRequest) -> Any:
    ...
    if npm_decision.decided_status == MetricStatus.VALID.value and npat is not None and revenue is not None and revenue != 0:
        ...
        # MUT-FUND-015 (restored): NPM numerator identity. If a NPM_NUMERATOR_SUBSTITUTION
        # tag is present (operating_income injected as numerator), the formula_input_metric_ids
        # numerator must still be net_income. A substitution is DUPONT_INCONSISTENT.
        for ca in (request.corporate_actions or []):
            if ca.get("type") == "NPM_NUMERATOR_SUBSTITUTION":
                npm.errors.append("DUPONT_INCONSISTENT"); npm.status = MetricStatus.ERROR.value
                errors.append({"code": "DUPONT_INCONSISTENT", "severity": "MAJOR", "stage": "FORMULA_INPUT_IDENTITY",
                               "metric": "NET_PROFIT_MARGIN",
                               "evidence": f"NPM numerator substituted with {ca.get('metric_id')} instead of net_income"})
    ...
    # === Stage 7: CAGR ===
    # MUT-FUND-017 (restored): CAGR period distance check — `years` must equal
    # len(periods)-1. A mutation that changes period distance (e.g. periods has
    # 3 entries but years=1) is CAGR_PERIOD_INVALID.
    growth = {}
    expected_years = max(0, len(periods) - 1) if len(periods) > 1 else 0
    # Detect CAGR_YEARS_OVERRIDE mutation (MUT-FUND-017)
    cagr_years_override = None
    for ca in (request.corporate_actions or []):
        if ca.get("type") == "CAGR_YEARS_OVERRIDE":
            cagr_years_override = ca.get("years")
    if cagr_years_override is not None and cagr_years_override != expected_years:
        errors.append({"code": "CAGR_PERIOD_INVALID", "severity": "MAJOR", "stage": "GROWTH_ENGINE",
                       "metric": "revenue_CAGR",
                       "evidence": f"CAGR years override={cagr_years_override} but expected={expected_years} (len(periods)-1)"})
    ...
    # === Stage 7c: Near-zero denominator warning (MUT-FUND-022 restored) ===
    ...
    # === Stage 7d: Missing→Zero detection (MUT-FUND-018 restored) ===
