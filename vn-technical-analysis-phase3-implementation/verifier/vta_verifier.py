...
# ===========================================================================
# Output-packet normalization (production envelope -> flat verifier view)
# ===========================================================================

# Production code (src/output_assembler.py) emits an ``OutputPacket`` as:
#   {mode, body: {schema_version, mode, symbol, as_of_date, analysis_status,
#                 indicators, tech_score, binary_signals_6, ...},
#    provenance: {...}, validation: {...}}
# which is itself often wrapped by the integration adapter as:
#   {ticker, mode, is_valid, packet: <OutputPacket.to_dict()>, boundary_check}
# The domain modules were written against the flat (harness-observed) shape
# used by the self-test: indicators/tech_score/mode/as_of_date at top level.
# This helper unwraps the nested envelope into that flat view without mutating
# the original artifact, so the verifier can accept BOTH formats. It is a pure
# structural transform (no production decision logic) and is a no-op on a
# packet that is already flat.
_BODY_KEYS_FLAT = (
    "indicators", "tech_score", "binary_signals_6", "six_binary_signals",
    "profile_blocks", "blocks", "setups", "archetype", "setup_coverage_status",
    "as_of_date", "analysis_status",
)


def _normalize_output_packet(packet: Any) -> Dict[str, Any]:
    """Flatten a production-shaped output packet into the flat view the domain
    modules were written against. Idempotent and side-effect free.

    Recognized shapes (in priority order):
      1. Adapter envelope: {ticker, mode, packet: {mode, body, provenance, ...}}
      2. OutputPacket.to_dict(): {mode, body: {...}, provenance: {...}, validation}
      3. Already-flat packet: {mode, indicators, tech_score, as_of_date, ...}

    For shapes 1 and 2 the ``body`` sub-dict is promoted to top level; the
    ``provenance``/``validation`` sub-dicts and any envelope-level keys
    (ticker, boundary_check, is_valid) are preserved alongside. Nested values
    present both in the body and at envelope level defer to the body (the
    authoritative assembled payload).
    """
    if not isinstance(packet, Mapping):
        return packet  # type: ignore[return-value]

    # Shape 1: adapter envelope wrapping an OutputPacket under 'packet'.
    inner = packet.get("packet")
    if isinstance(inner, Mapping) and isinstance(inner.get("body"), Mapping):
        merged = _flatten_body_packet(inner)
        # Carry over envelope-level context keys (ticker, is_valid,
        # boundary_check) that are not part of the body payload.
        for key, value in packet.items():
            if key == "packet":
                continue
            if key in ("mode", "body", "provenance", "validation"):
                # Prefer the body-derived values already in merged.
    ...
    # Shape 2: a bare OutputPacket.to_dict() with a nested body.
    if isinstance(packet.get("body"), Mapping):
        return _flatten_body_packet(packet)

    # Shape 3: already flat — return as-is (as a plain dict).
    return dict(packet)  # type: ignore[return-value]


def _flatten_body_packet(packet: Mapping[str, Any]) -> Dict[str, Any]:
    """Promote a {mode, body, provenance, validation} packet's body to top
    level while keeping provenance/validation siblings available."""
    merged: Dict[str, Any] = {}
    body = packet.get("body")
    if isinstance(body, Mapping):
        for key, value in body.items():
            merged[key] = value
    # Sibling keys (provenance, validation, mode) supplement body values; the
    # body wins when both carry the same key (body is the assembled payload).
    for key, value in packet.items():
        if key == "body":
            continue
        if key not in merged:
...
class VerificationContext:
    ...
    def __init__(
        ...
        *,
        ...
        self.output_packet = _normalize_output_packet(
            common.load_artifact(output_packet_path)
        )
...
def run_verification(
    ...
    *,
    ...
    # --- Correct execution matrix (Section 7) -------------------------------
    # Emit a result record ONLY for (fixture_id, VC_id) pairs where
    # VC_id ∈ fixture.target_VC_ids. The cross-product of every fixture
    # against all 64 VCs produces meaningless FAIL/ERROR noise; the canonical
    # coverage relation is target_relationships_only. Each canonical fixture
    # declares exactly which VCs it is contracted to address.
    target_vc_ids = _fixture_target_vc_ids(ctx)
    unknown_targets = sorted(set(target_vc_ids) - canonical_set)

    # Records are emitted for the fixture's declared targets. When a fixture
    # declares no targets (synthetic/harness fixtures) we fall back to the
    # canonical set so the structural self-test still observes all 64.
    emit_vc_ids: List[str] = (
        [vc for vc in canonical if vc in set(target_vc_ids)]
        if target_vc_ids
        else list(canonical)
    )

    records: List[Dict[str, Any]] = []
    for vc_id in emit_vc_ids:
    ...
    summary = _summarise(
        records, canonical, resolved_outcomes, ctx,
        target_vc_ids=target_vc_ids, unknown_targets=unknown_targets,
    )
...
def _fixture_target_vc_ids(ctx: VerificationContext) -> List[str]:
    """The frozen fixture's declared ``target_VC_ids``.

    Per Section 7 the canonical execution relation is
    ``(fixture_id, VC_id) where VC_id ∈ fixture.target_VC_ids``. Reading the
    target set from the frozen fixture (not a runtime-derived cross product)
    is what bounds the result-record count to exactly the target
    relationships. Returns an empty list when the fixture is absent or declares
    no targets (synthetic harness fixtures), in which case the caller falls
    back to the full canonical VC set.
    """
    ...
    if not isinstance(fixture, Mapping):
        return []
    targets = fixture.get("target_VC_ids") or []
...
def _summarise(
    ...
    ctx: VerificationContext,
    *,
    target_vc_ids: Optional[Sequence[str]] = None,
    unknown_targets: Optional[Sequence[str]] = None,
) -> Dict[str, Any]:
    ...
    # Execution-matrix accounting (Section 7): only target relationships are
    # canonical result records. Report the exact target count, any unknown
    # target VCs, and confirm no non-target pairs were executed.
    targets = list(target_vc_ids) if target_vc_ids is not None else []
    unknown = list(unknown_targets) if unknown_targets is not None else []
    target_set = set(targets)
    record_vc_ids = {r["VC_id"] for r in records}
    non_target_executed = sorted(
        record_vc_ids - target_set
    ) if target_set else []
    ...
    return {
        ...
        "execution_matrix": {
            "fixture_target_VC_ids": targets,
            "target_VC_relationships": len(targets),
            "VC_result_records": len(records),
            "non_target_fixture_VC_pairs_executed": len(non_target_executed),
            "non_target_VC_ids": non_target_executed,
            "unknown_target_VCs": unknown,
            "target_relationships_only": len(non_target_executed) == 0 and len(unknown) == 0,
        },
    }
