...
# The set also accommodates the production ``OutputPacket`` envelope: after
# normalization the body is promoted to top level, so body-native keys
# (schema_version, symbol) and envelope metadata keys (validation,
# boundary_check, is_valid) appear alongside the harness-observed keys.
_ACTIVE_ALLOWED_TOP = {
    "mode",
    "as_of_date",
    "ticker",
...
def _vc_bench_misalign_1(ctx, packet, fixture) -> CheckOutcome:
    ...
    benchmark_none = benchmark is None or _fixture_field(fixture, "benchmark_none") is True
    error_code = packet.get("error_code")
    status = packet.get("analysis_status")
    warnings = _provenance_warnings(packet)
    bench_warned = any(
