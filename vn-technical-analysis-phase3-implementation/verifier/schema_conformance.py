...
def _ohlcv_rows(fixture: Dict[str, Any]) -> List[Dict[str, Any]]:
    rows = fixture.get("ohlcv") or fixture.get("rows") or fixture.get("bars")
...
# descriptions; production schemas are not imported). These are intentionally
# permissive about sub-keys the contract calls out as required/optional.
_ACTIVE_ALLOWED_TOP = {
    "mode",
    "as_of_date",
    "ticker",
    ...
    if is_empty_case:
        if error_code == "INSUFFICIENT_HISTORY" and not has_partial_output:
            return CheckOutcome.fail(
                ...
                reason="empty input produced INSUFFICIENT_HISTORY with no partial output",
            )
        return CheckOutcome.fail(
            "INSUFFICIENT_HISTORY",
            ["EMPTY_SERIES_DETECTED"],
    ...
    if below:
        if error_code == "INSUFFICIENT_HISTORY":
            return CheckOutcome.fail(
                ...
                reason=f"{mode} input below {baseline}-session baseline correctly rejected",
                mode=mode,
...
def _vc_miss_int_1(ctx, packet, fixture) -> CheckOutcome:
    ...
    if not dates_seq:
        return CheckOutcome.error("no dated rows in fixture")
    expected = fixture.get("expected_trading_days") or fixture.get("expected_sessions")
    actual = len(dates_seq)
    if expected is None:
        # Infer expected from min/max date range (calendar trading days).
        try:
            from datetime import date as _d

            d_sorted = sorted(dates_seq)
            d0 = _d.fromisoformat(d_sorted[0][:10])
            d1 = _d.fromisoformat(d_sorted[-1][:10])
            expected = max((d1 - d0).days + 1, actual)
        except Exception:
            expected = actual
    try:
    ...
    if ratio < 0.95:
        if status == "VALID_WITH_WARNINGS" and gap_warned:
            return CheckOutcome.fail(
                ...
                reason="gap ratio below 0.95 correctly warned",
                actual_days=actual,
    ...
    if zero_fields:
        if error_code == "ZERO_PRICE_DETECTED" and not has_partial:
            return CheckOutcome.fail(
                "ZERO_PRICE_DETECTED",
                ["ZERO_PRICE_DETECTED"],
                reason="zero price correctly rejected",
...
def _vc_bench_misalign_1(ctx, packet, fixture) -> CheckOutcome:
    ...
    benchmark = fixture.get("benchmark") or fixture.get("benchmark_series")
    benchmark_none = benchmark is None or fixture.get("benchmark_none") is True
    error_code = packet.get("error_code")
    status = packet.get("analysis_status")
    warnings = _provenance_warnings(packet)
    bench_warned = any(
    ...
    overlap = fixture.get("overlap_ratio")
    if benchmark_none:
...
def _vc_partial_week_1(ctx, packet, fixture) -> CheckOutcome:
    ...
    if not str(fixture.get("mode", "")).upper().startswith("ACTIVE") and (
        _detect_mode(packet) != "ACTIVE"
    ):
        # Not an ACTIVE case; VC is silent.
        return CheckOutcome.pass_clean(active_mode=False)
    partial = fixture.get("partial_week") or fixture.get("latest_week_trading_days")
    is_partial = (
        partial is not None
        and _is_finite(partial)
        and float(partial) < 5
    ) or fixture.get("has_partial_week") is True
    status = packet.get("analysis_status")
    warnings = _provenance_warnings(packet)
...
def _vc_profile_valid_3(ctx, packet, fixture) -> CheckOutcome:
    ...
    blocks = packet.get("profile_blocks") or packet.get("blocks") or []
...
def _vc_mode_sep_1(ctx, packet, fixture) -> CheckOutcome:
    ...
    if _detect_mode(packet) != "ACTIVE":
        return CheckOutcome.pass_clean(non_active_packet=True)
    if any(k in packet for k in ("profile_blocks", "blocks", "setups", "archetype")):
        return CheckOutcome.fail(
            "MODE_CONTAMINATION",
            ["PROFILE_BLOCKS_IN_ACTIVE"],
            reason="ACTIVE output contains profile-only keys",
            present=[k for k in ("profile_blocks", "blocks", "setups", "archetype") if k in packet],
        )
    return CheckOutcome.pass_clean()
...
def _vc_mode_sep_2(ctx, packet, fixture) -> CheckOutcome:
    ...
    present = [k for k in forbidden if k in packet]
    if present:
        return CheckOutcome.fail(
            "MODE_CONTAMINATION",
...
def _vc_mode_contam_1(ctx, packet, fixture) -> CheckOutcome:
    ...
    Negative-fixture companion to VC-MODE-SEP-1."""
    if _detect_mode(packet) != "ACTIVE":
        return CheckOutcome.pass_clean(non_active_packet=True)
    contamination = [k for k in ("profile_blocks", "blocks", "setups", "archetype") if k in packet]
    if contamination:
