...
def _numeric_output_fields(packet: Dict[str, Any]) -> List[Tuple[str, float]]:
    ...
    provenance/warnings/errors envelopes themselves (they carry metadata, not
    output values requiring traceability)."""
    out: List[Tuple[str, float]] = []
    skip = {"provenance", "warnings", "errors", "computation_chain"}
...
def _field_has_provenance(packet: Dict[str, Any], field_path: str) -> bool:
    ...
    for node in chain:
        ...
        if isinstance(node_fields, str) and node_fields == field_path:
            return True
    # Field-path prefix match (e.g. indicators.RSI.rsi_value matches a chain
    # node tagged indicators.RSI).
...
def _is_exempt_presentation_field(path: str) -> bool:
    ...
    if head in {"weekly_history", "daily_history", "as_of_date", "mode", "ticker"}:
        return True
    if path in {"weekly_history", "daily_history"}:
        return True
    return False
