...
def _vc_rsi_wilder_1(ctx, packet, fixture) -> CheckOutcome:
    ...
    if not closes or not engine_rsi:
        return CheckOutcome.error(
            "missing close series or engine RSI series",
    ...
    if isinstance(reference, dict):
        for key in ("rsi_series", "reference_series", "values", "series"):
            ...
            if isinstance(v, list):
                ...
                break
    if not ref_series:
        # Without a frozen reference the calibration gate cannot run; report
        # ERROR rather than silently passing.
        return CheckOutcome.error(
            "frozen RSI reference fixture not found; calibration gate cannot run",
...
def _vc_wrong_smooth_1(ctx, packet, fixture) -> CheckOutcome:
    ...
    if not closes or not engine_rsi:
        return CheckOutcome.error(
            "missing close series or engine RSI series for smoothing detection"
        )
...
def _vc_boll_1(ctx, packet, fixture) -> CheckOutcome:
    ...
    if not closes or bb is None:
        return CheckOutcome.error(
            "missing close series or BB indicator in packet"
        )
    ...
    if not engine_middle:
        ...
        for k in ("lower_value", "lower"):
            ...
            if _is_finite(v):
                ...
                break
    if not engine_middle:
        return CheckOutcome.error("BB indicator has no middle band values")
    expected_upper_pop = [
    ...
    frozen_match = drift_pop <= _TOL_COMPARE
    convention_tag_ok = convention in (None, "POPULATION_DIV_N", "POPULATION")
    if not frozen_match and drift > _TOL_COMPARE:
        return CheckOutcome.fail(
...
def _assert_active_profile_bb_identical(
        ...
        if isinstance(indicators, dict):
            ...
            for node in indicators:
                if isinstance(node, dict) and node.get("id") in ("BB", "Bollinger"):
                    bb_profile = node
                    break
    if bb_active is None and bb_profile is None:
        # If only one mode is present in the packet, kernel equality is
        # trivially satisfied for the present mode against the recompute.
        recompute_middle = simple_moving_average(closes, 20)
        present = bb_active or bb_profile
        if present is None:
            return CheckOutcome.error(
                f"{vc_id}: no BB indicator present in either mode envelope"
            )
    ...
    if bb_active is None or bb_profile is None:
        return CheckOutcome.fail(
            ...
            ["ACTIVE_PROFILE_BB_DIVERGE"],
            reason=(
                "only one of ACTIVE/PROFILE BB present; shared-kernel equality "
...
def _vc_channel_1(ctx, packet, fixture) -> CheckOutcome:
    ...
    if not closes or observed is None:
        return CheckOutcome.error(
            "missing close series or channel slope observation",
            closes_present=bool(closes),
...
def _vc_channel_2(ctx, packet, fixture) -> CheckOutcome:
    ...
    if frozen_threshold is None:
        return CheckOutcome.error(
            "frozen channel-slope threshold not found in formula contracts",
            looked_in="F-CHANNEL-SLOPE / T-CHANNEL-SLOPE",
        )
    if applied is None:
        return CheckOutcome.fail(
...
def _vc_obv_vpt_1(ctx, packet, fixture) -> CheckOutcome:
    ...
    if obv_change_observed is None and vpt_change_observed is None:
        return CheckOutcome.error(
            "neither obv_change nor vpt_change present in output",
        )
...
def _vc_price_basis_1(ctx, packet, fixture) -> CheckOutcome:
    ...
    if not chain:
        return CheckOutcome.error(
            "no computation_chain in packet; cannot verify price_basis tagging"
        )
    untagged: List[str] = []
    invalid: List[tuple] = []
...
def _vc_price_basis_2(ctx, packet, fixture) -> CheckOutcome:
    ...
    if not chain:
        return CheckOutcome.error(
            "no computation_chain in packet; cannot verify returns/beta basis"
        )
