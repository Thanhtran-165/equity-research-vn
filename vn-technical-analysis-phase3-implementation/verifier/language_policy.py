...
def _collect_text(packet: Dict[str, Any]) -> List[Tuple[str, str]]:
    ...
    provenance/warnings/errors envelopes and structural identifiers."""
...
def _looks_like_phrase(text: str) -> bool:
    ...
    if not stripped:
        return False
    # All-caps enum identifiers (e.g. VALID_WITH_WARNINGS) are structural, not
    # natural language. But the policy still scans them for prohibited tokens;
    # the negation/semantic layers handle context.
