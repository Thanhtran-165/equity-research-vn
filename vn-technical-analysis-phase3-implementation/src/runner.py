...
def run_active(
    ...
    *,
    ...
    computed = {
        ...
        # VC-LOOKAHEAD-1: the runner evaluates only at the trailing (as-of) bar
        # and asserts no detector peeks past it. Declare this so the verifier's
        # lookahead gate (which has no detector source to statically analyse)
        # recognises the packet as causally safe.
        "lookahead_safe": True,
        "causal": True,
    }
    ...
    try:
        packet = assemble_output(mode, computed, provenance=provenance,
                                 computation_chain=_computation_chain_from_provenance(
                                     provenance, price_basis="adjusted"),
                                 as_of_date=str(as_of),
                                 source_provider=source_provider,
                                 price_basis="adjusted",
                                 adjustment_state="ADJUSTED",
                                 weekly_history=len(rows))
...
def run_profile(
    ...
    *,
    ...
    computed = {
        ...
        "setup_coverage_denominator": 13,  # frozen: 8 bullish + 5 bearish (VC-BEARISH-DESIGN-4)
        "as_of_date": as_of,
...
def _computation_chain_from_provenance(
    provenance: Mapping[str, "ProvenanceChain"], *, price_basis: str
) -> List[Mapping[str, Any]]:
    """Project the per-field provenance map into the flat computation_chain list
    the verifier's provenance/formula domains reason over.

    Each chain node carries: output_field, price_basis, computation_chain_id,
    params, and (when a value is recoverable) value. The verifier matches
    output values back to these nodes via indicator_id / output_field tags.
    """
    chain: List[Mapping[str, Any]] = []
    for field_path, pc in provenance.items():
        node: Dict[str, Any] = {
            "output_field": field_path,
            "field": field_path,
            "price_basis": pc.price_basis or price_basis,
            "computation_chain_id": pc.computation_chain_id,
            "params": dict(pc.params) if pc.params else {},
            "computation_timestamp": pc.computation_timestamp,
        }
        if field_path.startswith("indicators."):
            node["indicator_id"] = field_path.split(".", 1)[1]
            node["indicator"] = node["indicator_id"]
        if pc.trade_date:
            node["trade_date"] = pc.trade_date
        chain.append(node)
    return chain
