...
def _gen_chain_id() -> str:
    """Deterministic-ish chain id (caller may override in provenance).

    Uses uuid4 deterministically per-process-free semantics; no wall clock.
    The contract requires reproducible-across-runs for numeric output; the
    chain id is metadata only and is not load-bearing for determinism of
    the numeric fields.
    """
    return "chain-deterministic-0000000000000000"
...
def normalize_input(
    ...
    *,
    ...
    chain = chain_id or _gen_chain_id()
