...
PROFILE_FORBIDDEN_KEYS: Tuple[str, ...] = (
    ...
    "binary_signals_6",
    "six_binary_signals",
    "indicators",   # ACTIVE-only aggregate
)
...
_ACTIVE_SCHEMA = OutputSchema(
    ...
    allowed=("schema_version", "mode", "symbol", "indicators", "tech_score",
             "binary_signals_6", "as_of_date", "analysis_status",
             "error_code", "warnings"),
    numeric=("tech_score",),
)
...
_PROFILE_SCHEMA = OutputSchema(
    ...
    allowed=("schema_version", "mode", "symbol", "profile_blocks",
             "archetype", "setup_coverage_status", "as_of_date",
             "analysis_status", "error_code", "warnings",
             # Setup listings are PROFILE-legal (pattern observations, not ACTIVE).
...
def assemble_output(
    ...
    *,
    ...
    body: Dict[str, Any] = {
        ...
        "as_of_date": as_of_date or computed_results.get("as_of_date", "UNKNOWN"),
        "analysis_status": computed_results.get("analysis_status", "VALID"),
    }
    if "error_code" in computed_results:
        body["error_code"] = computed_results["error_code"]
    if computed_results.get("warnings"):
        body["warnings"] = list(computed_results["warnings"])

    if mode == MODE_ACTIVE:
        body["indicators"] = computed_results.get("indicators", {})
        body["tech_score"] = computed_results.get("tech_score")
        body["binary_signals_6"] = computed_results.get("binary_signals_6", [])
    else:  # PROFILE
        body["profile_blocks"] = computed_results.get("profile_blocks", {})
        body["archetype"] = computed_results.get("archetype", "A-NO-CURRENT-SETUP")
        body["setup_coverage_status"] = computed_results.get(
            "setup_coverage_status", "INCOMPLETE_BEARISH_COVERAGE")
        # Optional bearish/bullish setup listings (still PROFILE-legal).
        if "bullish_setups" in computed_results:
            body["bullish_setups"] = computed_results["bullish_setups"]
        if "bearish_setups" in computed_results:
            body["bearish_setups"] = computed_results["bearish_setups"]
