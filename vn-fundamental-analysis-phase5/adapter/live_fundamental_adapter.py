...
PHASE4R3_IMPL = Path(__file__).parent.parent.parent / "vn-fundamental-analysis-phase4R3" / "implementation"
...
def adapt_ticker(ticker: str, years: List[int], peer_set: Optional[List] = None) -> Tuple[FundamentalRequest, Dict]:
    ...
    if any(v is not None for v in shares_norm_list):
        metrics["shares_outstanding"] = MetricInput(
            ...
            source_id=f"vnstock:{ticker}:weighted_avg_basic_shares",
            ...
            source_metric_ids=[f"vnstock_weighted_shares_backcalc_{y}" for y in years],
            source_dates=[fetch_ts]*n,
            source_types=["live_provider_vnstock_backcalc"]*n,
        )
