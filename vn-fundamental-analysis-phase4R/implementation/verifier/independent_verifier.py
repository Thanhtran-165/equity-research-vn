...
def verify(request: FundamentalRequest, output: FundamentalOutput) -> VerificationResult:
    ...
    for mid, denom_metric in [("ROE", eq), ("ROA", ta), ("DUPONT_EM", eq)]:
    ...
    roa_obj = next((m for m in output.metric_results if m.metric_id == "ROA"), None)
    if roa_obj and roa_obj.status in (MetricStatus.VALID.value, MetricStatus.VALID_NEGATIVE.value):
        ...
        if denom_metric_id and denom_metric_id == "equity":
            if "DUPONT_INCONSISTENT" not in roa_obj.errors:
                fail("DUPONT_INCONSISTENT", "ROA", "ROA denominator metric_id is equity, not total_assets")

    # === Structural: Period alignment (MUT-FUND-023) ===
    for mid, num, denom in [("ROE", npat, eq), ("ROA", npat, ta), ("NET_PROFIT_MARGIN", npat, rev)]:
    ...
    for mid, num, denom, allow_attr in [("ROE", npat, eq, False), ("ROA", npat, ta, True), ("NET_PROFIT_MARGIN", npat, rev, False)]:
        if num is None or denom is None:
            continue
        ok, err, _ = validate_scope_alignment(num, denom, latest, allow_attribution_mismatch=allow_attr)
    ...
    for mid in ("EPS_BASIC", "BVPS", "ROE", "ROA", "NET_PROFIT_MARGIN"):
