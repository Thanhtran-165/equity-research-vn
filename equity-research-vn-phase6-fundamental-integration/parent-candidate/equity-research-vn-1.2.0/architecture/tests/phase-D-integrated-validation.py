...
for case_id, desc, mutate_fn, owner in corruptions:
    ...
    if not ok:
        ...
        record(case_id, "corrupted", "FPT", "RENDERED_BUT_DETECTABLE", "RENDERED_BUT_DETECTABLE", owner, {"note": desc})
...
for i, sec_id in enumerate(isolation_sections):
    ...
    record(f"ISO-{i+1}", "section_isolation", "FPT", "ISOLATED", "ISOLATED" if ok else "CASCADE", "SECTION_VALIDATOR",
