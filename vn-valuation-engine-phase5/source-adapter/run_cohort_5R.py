"""Phase 5R cohort runner — fresh requalification with patched adapter."""
...
ARTIFACTS = Path(WORK) / "artifacts" / "phase5R-genuine-baseline"
...
def run_one(ticker, label):
    ...
    json.dump({"ticker":ticker,"run_label":label,"cohort":"phase5R",
    ...
    summary = {
        "ticker": ticker, "run_label": label, "cohort": "phase5R",
        "final_status": fs,
...
for ticker in TICKERS:
    for label in ["run-A","run-B"]:
        try:
            ...
            time.sleep(10)  # rate-limit avoidance (more conservative)
        except Exception as e:
            print(f"FATAL {ticker}-{label}: {e}")
            all_runs.append({"ticker":ticker,"run_label":label,"cohort":"phase5R","final_status":"FATAL","error":str(e)})
            time.sleep(30)

out = Path(WORK) / "manifests" / "phase5R-run-manifest.json"
json.dump({"phase_5R_runs": {
    "generated_at": dt.datetime.now(dt.timezone.utc).isoformat(),
    "total_runs": len(all_runs),
    "runs": all_runs,
    "adapter_version": "v2 (Scale.UNIT fixed) — Phase 5R patched",
    "cohort_type": "FRESH_REQUALIFICATION",
    "no_reuse_from_phase_5_raw": True,
}}, open(out,'w'), indent=2, default=str, ensure_ascii=False)
print(f"\n=== PHASE 5R COHORT DONE: {len(all_runs)} runs ===")
print(f"WROTE {out}")
