...
def run_fundamental(request: FundamentalRequest) -> Any:
    ...
    # Near-zero equity relative to assets (directive §3: near_zero_relative_to_assets)
    # If common_equity < 5% of total_assets, flag as EXTREME_EQUITY_RATIO_WARNING
    if equity is not None and assets is not None and assets != 0:
        equity_ratio = abs(equity) / abs(assets)
        if equity_ratio < 0.05 and equity > 0:
            for m in metric_results:
                if m.metric_id in ("ROE", "BVPS"):
                    warning = f"EXTREME_EQUITY_RATIO_WARNING: equity/assets={equity_ratio:.4f} < 5%"
                    if warning not in m.warnings:
                        m.warnings.append(warning)
    ...
    # Determine PE applicability: NOT_APPLICABLE when EPS <= 0 (negative or zero)
    eps_value_for_pe = eps_result.value if eps_result else None
    pe_applicability = "NOT_APPLICABLE" if (eps_value_for_pe is not None and eps_value_for_pe <= 0) else "APPLICABLE"
    ...
    downstream_export = {
        "EPS_basic": {
            ...
            "quality_status": getattr(eps_result, 'quality_status', 'VALID') if eps_result else None,
