...
def verify(request: FundamentalRequest, output: FundamentalOutput) -> VerificationResult:
    ...
    # 5. Negative EPS sign consistency: verifier independently recomputes EPS sign.
    # If engine says positive but NPAT is negative, that's a sign flip.
    eps_obj_sign = next((m for m in output.metric_results if m.metric_id == "EPS_BASIC"), None)
    if eps_obj_sign and eps_obj_sign.status == MetricStatus.VALID.value and eps_obj_sign.value is not None:
        if npat_v is not None and shares_v is not None and shares_v != 0:
            expected_eps = npat_v / shares_v
            if expected_eps < 0 and eps_obj_sign.value > 0:
                fail("EPS_SIGN_INCONSISTENT", "EPS_BASIC",
                     f"EPS={eps_obj_sign.value} >0 but NPAT/shares={expected_eps} <0 — sign flipped")

    # 6. Negative EPS suppression: if decision was VALID but status is INPUT_INCOMPLETE,
    # check if NPAT+shares were actually available (suppression).
    if eps_obj_sign and eps_obj_sign.status == MetricStatus.INPUT_INCOMPLETE.value:
        if npat_v is not None and shares_v is not None and shares_v != 0:
            # Inputs were available → INPUT_INCOMPLETE is suspicious (suppression)
            fail("DOWNSTREAM_EXPORT_BLOCKED", "EPS_BASIC",
                 "EPS is INPUT_INCOMPLETE but NPAT and shares are available — negative EPS suppressed")

    # 7. EXTREME_EQUITY_RATIO_WARNING presence check
    if eq_v is not None and ta_v is not None and ta_v != 0 and eq_v > 0:
        eq_ratio = abs(eq_v) / abs(ta_v)
        if eq_ratio < 0.05:
            for mid_check in ("ROE", "BVPS"):
                m_check = next((m for m in output.metric_results if m.metric_id == mid_check), None)
                if m_check and m_check.status in (MetricStatus.VALID.value, MetricStatus.VALID_NEGATIVE.value):
                    has_warn = any("EXTREME_EQUITY_RATIO_WARNING" in w for w in m_check.warnings)
                    if not has_warn:
                        fail("REQUIRED_WARNING_REMOVED", mid_check,
                             f"equity/assets={eq_ratio:.4f} <5% but EXTREME_EQUITY_RATIO_WARNING absent")

    # 8. Circular EPS oracle detection: if output note claims provider EPS as verification
    for m in output.metric_results:
        if m.note and "circular" in m.note.lower():
            fail("CIRCULAR_ORACLE_DETECTED", m.metric_id,
                 f"metric note indicates circular oracle: {m.note}")
